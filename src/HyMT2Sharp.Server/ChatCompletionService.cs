...
public sealed class ChatCompletionService : IDisposable
{
    ...
    public ChatCompletionService(string modelPath, int threads, int defaultMaxTokens = 256, KvCacheConfig? cacheConfig = null)
    {
    ...
    public async Task<ChatCompletionResponse> CompleteAsync(ChatCompletionRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            if (!ok || !_model.CacheConfig.KeepFlatCache)
                _model.EndRequest();
            _gate.Release();
        }
    }
